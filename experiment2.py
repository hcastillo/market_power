...
class Experiment2:
    ...
    def next(values=None):
        if len(next(iter(Experiment2.parameters.values()))) != 3:
            return Experiment2.next_list(values)
        else:
            if not values:
                values = []
                for i in Experiment2.parameters:
                    values.append([i, Experiment2.parameters[i][1]])
                return values
            else:
                i = 0
                while i < len(values):
                    if values[i][1] >= Experiment2.parameters[values[i][0]][2]:
                        values[i][1] = Experiment2.parameters[values[i][0]][1]
                        i = i + 1
                    else:
                        values[i][1] += Experiment2.parameters[values[i][0]][0]
                        return values
                return []
    ...
    def next_list(values):
        if not values:
            values = []
            for i in Experiment2.parameters:
                values.append([i, Experiment2.parameters[i][0]])
            return values
        else:
            i = 0
            while i < len(values):
                j = 0
                while j < len(Experiment2.parameters[values[i][0]]) - 1:
                    if values[i][1] == Experiment2.parameters[values[i][0]][j]:
                        values[i][1] = Experiment2.parameters[values[i][0]][j + 1]
                        for ii in range(i):
                            values[ii][1] = Experiment2.parameters[values[ii][0]][0]
                        return values
                    j = j + 1
                i = i + 1
            return []
    ...
    @staticmethod
    def num_models():
        values = Experiment2.next()
        i = 0
        while values:
            i += 1
            values = Experiment2.next(values)
        return i

    @staticmethod
    def empty_array_for_results(is_numpy=True):
        result = {}
        for i_analyze_data in Experiment2.analyze_data:
    ...
    @staticmethod
    def do(model: Model):
        log_experiment = open(f'{Experiment2.output}experiment1.txt', 'w')
        num_models_analyzed = 0
        model.test = False
        values = Experiment2.next()
        progress_bar = Bar('Executing models', max=Experiment2.num_models())
        print(f"# model <id> <parameters> <results, {Experiment2.analyze_data}>", file=log_experiment)
        results_to_plot = Experiment2.empty_array_for_results(is_numpy=False)
        results_x_axis = []
        while values:
            result_iteration = Experiment2.empty_array_for_results()
            for i in range(Experiment2.MC):
                mc_iteration = ["default_seed", random.randint(100, 10000)]
                values.append(mc_iteration)
                result_mc = Experiment2.run_model(model,
                                                  f"{Experiment2.output}experiment1_{num_models_analyzed}_{i}.csv",
                                                  values)
            ...
            progress_bar.next()
        Experiment2.plot(results_to_plot, results_x_axis, f"{Experiment2.output}experiment1")
